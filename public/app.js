...
if ($("unload-model")) {
  $("unload-model").addEventListener("click", async () => {
    if (state.analysis.active) return;
    try {
      window.__censorStationWebDetector?.unload?.();
    } catch (error) {
    ...
    setNotice(t("notice.modelUnloaded"));
  });
}
